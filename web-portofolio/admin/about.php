<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About</title>
</head>
<body data-bs-spy="scroll" data-bs-target=".navbar">

    <?php
        require_once "mainmenu.php";
    ?>

    <div id="content-wrapper">
        <!-- ABOUT -->
        <section id="about" class="full-height px-lg-5">
            <div class="container">

                <div class="row pb-4" data-aos="fade-up">
                    <div class="col-lg-8">
                        <h6 class="text-brand">ABOUT</h6>
                        <h1>My Education & Experiance</h1>
                    </div>
                </div>
                <?php
                    if (!koneksi()){
                        die("Koneksi Error");
                    }
                    $querySQL = "SELECT * FROM about";
                    $execQuerySQL = mysqli_query(koneksi(),$querySQL);

                    if(mysqli_num_rows($execQuerySQL) > 0){
                        while($row = mysqli_fetch_assoc($execQuerySQL)){
                ?>
                <div class="row gy-5">
                    <div class="col-lg-6">

                        <h3 class="mb-4" data-aos="fade-up" data-aos-delay="300">Education</h3>
                        <div class="row gy-4">

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['edukasi1'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_edukasi1'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['edukasi2'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_edukasi2'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['edukasi3'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_edukasi3'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                        </div>

                    </div>

                    <div class="col-lg-6">

                        <h3 class="mb-4" data-aos="fade-up" data-aos-delay="300">Certification</h3>
                        <div class="row gy-4">

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['sertifikasi1'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_sertifikasi1'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['sertifikasi2'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_sertifikasi1'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                            <div class="col-12" data-aos="fade-up" data-aos-delay="600">
                                <div class="bg-base p-4 rounded-4 shadow-effect">
                                    <h4><?= $row['sertifikasi3'] ?></h4>
                                    <p class="text-brand mb-2"><?= $row['deskripsi_sertifikasi1'] ?></p>
                                    <p class="mb-0">All we are more and design lorem ipsum dolor creativity sit amet consectetur adipisicing elit</p>
                                </div>
                            </div>

                            <?php
                                    }
                                }
                            ?>

                        </div>

                    </div>

                </div>

            </div>
        </section>
        <!-- //ABOUT -->

        <!-- FOOTER -->
        <footer class="py-5 px-lg-5">
            <div class="container">
                <div class="row gy-4 justify-content-between">
                    <div class="col-auto">
                    <p class="mb-0">Created With Love By <a href="#" class="fw-bold">Abimanyu Luthfi</a></p>
                    </div>
                    <div class="col-auto">
                        <div class="social-icons">
                            <a href="https://wa.me/6282181677382?text=Hi%20I'am%20Abimanyu%20Luthfi%20Rizq%20Ramadhan,%20Junior%20Programmer%20From%20Pembangunan%20Jaya%20University,%20You%20Have%20Any%20Question?%20Let's%20Talk%20To%20Me%20:)"><i class="lab la-whatsapp"></i></a>
                            <a href="https://www.instagram.com/abimanyuluthfi25/"><i class="lab la-instagram"></i></a>
                            <a href="https://github.com/AbimanyuLuthfi"><i class="lab la-github"></i></a>
                            <a href="#"><i class="lab la-linkedin"></i></a>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
        <!-- //FOOTER -->
    </div>
</body>
</html>